import axios from "axios";
import { useState } from "react";
import { Box, Input, Page, Select, Text, useNavigate } from "zmp-ui";
import AppHeader from "@/components/header";
import { TrueFalseTHPTStatement as Statement } from "@/components/teacher/question/true-false-THPT-statement";

export default function TrueFalseTHPTQuestion() {
  const navTo = useNavigate();
  const [questionList, setQuestionList] = useState([]);
  const { TextArea } = Input;

  return (
    <Page className="page page-wo-footer bg-white">
      <AppHeader title="Thêm câu hỏi Đúng – Sai (THPT)" showBackIcon />

      <form className="pt-4" noValidate>
        <Input
          label={<Text>Tiêu đề câu hỏi <span className="zaui-text-red-50">*</span></Text>}
          placeholder="Tiêu đề câu hỏi" required
        />

        <Box>
          <label>
            <Text className="mt-2">Hình ảnh minh họa</Text>
          </label>
          <input
            type="file"
            className="p-4 border zaui-border-gray-30 rounded-lg w-full text-center mt-2"
            accept="image/png, image/jpeg"
          />
        </Box>

        <Input
          label={<Text className="mt-2">Tiêu đề đoạn văn (trích đoạn)</Text>}
          placeholder="Tiêu đề đoạn văn"
        />

        <TextArea        
          label={<Text className="mt-2">Nội dung đoạn văn</Text>}
          placeholder="Nội dung đoạn văn"
        />

        <Input
          label={<Text className="mt-2">Tác giả và nguồn của đoạn văn</Text>}
          placeholder="Tác giả và nguồn của đoạn văn"
        />
        
        <Select
          label={<Text className="mt-2">Lớp <span className="zaui-text-red-50">*</span></Text>}
          defaultValue={-1} closeOnSelect
        >
          <Select.Option value={-1} title="Lớp" disabled className="zaui-text-red-50" />
          <Select.Option value={6} title="Lớp 6" />
          <Select.Option value={7} title="Lớp 7" />
          <Select.Option value={8} title="Lớp 8" />
          <Select.Option value={9} title="Lớp 9" />
          <Select.Option value={10} title="Lớp 10" />
          <Select.Option value={11} title="Lớp 11" />
          <Select.Option value={12} title="Lớp 12" />
        </Select>

        <Select
          label={<Text className="mt-2">Chủ đề <span className="zaui-text-red-50">*</span></Text>}
          defaultValue={-1} closeOnSelect
        >
          <Select.Option value={-1} title="Chủ đề" disabled className="zaui-text-red-50" />
          <Select.Option value="0" title="Lớp 6" />
          <Select.Option value="1" title="Lớp 7" />
        </Select>

        <Box className="mt-2">
          <Box className="flex items-center">
            <Text className="flex-1">Danh sách mệnh đề <span className="zaui-text-red-50">*</span></Text>
            <button className="zaui-bg-blue-80 text-white rounded-full py-1 px-4">
              Thêm mệnh đề mới
            </button>
          </Box>
          <hr />

          <Statement statement={1} />
          <Statement statement={2} />
          <Statement statement={3} />
          <Statement statement={4} />
          <hr />
        </Box>

        <div className="flex gap-x-2 justify-center mt-2">
          <input type="submit" value="Lưu" className="zaui-bg-blue-80 text-white rounded-full py-2 px-8" />
          <input type="button" value="Hủy" className="zaui-bg-blue-20 zaui-text-blue-80 rounded-full py-2 px-8" onClick={() => navTo("/teacher/question")} />
        </div>
      </form>
    </Page>
  )
}